...
{
    class SocketUDP : SocketFP
    {
        private Socket fp;
        private EndPoint endPoint;
        private EndPoint reciverP;

        public SocketUDP(String ip, int port,  int rxque_max = 10, int rxdata_len = 128)
        {
            DB_FLG = "[SocketUDP]";
            try
            {
                endPoint = new IPEndPoint(IPAddress.Parse(ip), port);
                reciverP = new IPEndPoint(IPAddress.Any, 0);
                fp = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
                fp.Bind(new IPEndPoint(IPAddress.Any, 0));
        ...
        private void recv_proc()
        {
            ...
            try
            {
                while (is_err == false)
                {
                    ...
                    int data_len = fp.ReceiveFrom(rx_data,ref reciverP);
